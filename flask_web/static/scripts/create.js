$(document).ready(function () {
  ...
  $('#append').on('click', '.add-btn', function () {
    ...
    optionsDiv.append(
      `<input type="text" autofocus placeholder="Option ${optionCounter}" class="form-control option m-2">`
    )
    optionCounter++
  ...
  function gatherQuestionDetails () {
    ...
    if (!title || !description) {
      ...
      $('#confirmationModal .modal-body').html(
        'Please fill in all required Survey detail fields'
      )
      return (pause = True)
    } else {
      title = title.trim()
      ...
      $('.border.container').each(function () {
        ...
        if (!questionText) {
          ...
          showWarningModal()
          $('#confirmationModal .modal-body').html(
            'Please fill in  or delete empty question fields'
          )
          return (pause = True)
        } else {
    ...
    if (surveyData === 'continue') {
      ...
      saveSurvey(surveyData)
        .then(response => {
          ...
          $('#modal-cont')
            .removeClass('bg-success-subtle bg-warning-subtle')
